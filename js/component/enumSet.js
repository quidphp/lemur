...
(function ($, document, window) {
	...
	$.fn.enumSet = function()
	{
		...
		.on('enumSet:getSelect', function(event) {
		...
		.on('enumSet:isChoiceIn', function(event,value) {
			return (($.isScalar(value)) && $(this).triggerHandler('enumSet:getRadioCheckbox').filter("[value='"+value+"']").length)? true:false;
		})
	...
	$.fn.enumSetInput = function()
	{
		...
		.on('enumSetInput:getSelect', function(event) {
			return $(this).triggerHandler('enumSetInput:getParent').triggerHandler('enumSet:getSelect');
		})
		.on('ajax:getHref', function(event) {
			var parent = $(this).triggerHandler('enumSetInput:getParent');
			var select = $(this).triggerHandler('enumSetInput:getSelect');
		...
		.on('ajax:error', function(event,jqXHR,textStatus,errorThrown) {
			$(this).triggerHandler('enumSetInput:getResult').html($.parseError(jqXHR,textStatus));
		})
	...
	$.fn.enumSetFull = function()
	{
		...
		$(this).each(function(index, el) {
			...
			$(this).triggerHandler('enumSet:getSelect').on('change', function(event) {
				setTimeout(function() {
